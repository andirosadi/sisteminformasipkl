      ...
      @endif -->
                ...
                <div class="card shadow-sm" style="border-radius:10px!important;">
                    <div class="card-body wizard-content">
                        <h4 class="card-title">Edit Anggaran PKL</h4>
                        ...
                        @endif
                        <form action="{{url('a_laporananggaran/'.$files->id)}}" method="post" enctype="multipart/form-data">
                            {{csrf_field()}}
                            {{method_field('PATCH')}}
                            <div>
                                <section>
                                    <label for="judul">Judul</label>
                                    <input id="judul" name="judul" type="text" class="required form-control" value="{{$files->judul}}">
                                    <div class="card">
                                        <div class="card-body">
                                            <div for="laporananggaran" class="card-title">Anggaran PKL
                                            <P><input type="file" onchange="return validasiEktensi()" data-toggle="tooltip" title="Unggah file dengan ektensi PDF, xls atau xlsx. Kapasitas maksimal file yang diupload adalah 5MB" class="form-control-file" id="laporananggaran" name="laporananggaran" required></P>
                                            </div>
                                        </div>
                                    </div>
                                </section>
