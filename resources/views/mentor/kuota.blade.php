                    ...
                    <div id="zero_config_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4">
                        <div class="row">
                            <div class="col-sm-12 col-md-6">
                                <h5 class="card-title m-b-0">Data Kuota</h5>
                            </div>
                            <div class="col-sm-12 col-md-6">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <table class="table">
                                    ...
                                    <tbody id="myTable">
                                        @if(count($admin) != 0)
                                        @foreach($admin as $key => $kuota)
                                        <tr>
                                            <th scope="row"><b>{{$key+1}}</b></th>
                                            <td>{{$kuota->namadivisi}}</td>
                                            <td>{{$kuota->jumlahkuota}} Peserta</td>
                                            <td>{{\Carbon\Carbon::parse($kuota->created_at)->format('d F Y')}}</td>
                                            <td>
                                                @if($kuota->statuskuota_id == 1)
                                                    <i class="mdi mdi-check-circle" style="color: mediumseagreen">Disetujui</i>
                                                    @elseif($kuota->statuskuota_id == 2)
                                                        <i class="mdi mdi-clock" style="color: orange">Belum Disetujui</i>
                                                        @endif
                                            </td>
                                            <td>
                                                <div class="card-title m-b-0">
                                                    <form action="{{url('m_kuota/'.$kuota->id)}}" method="post">
                                                        <a href="{{url('m_kuota/'.$kuota->id).'/edit'}}" class="btn btn-warning btn-sm">Edit</a>
